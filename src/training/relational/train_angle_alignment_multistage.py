...
from src.models.quantum.relational.freezing import (
    load_param_dict,
    freeze_circuit_from_dict,
)
...
# ============================================================
# Multi-stage training: freeze shape parameters
# ============================================================

# Path to parameters learned from single-object training
# (update this path if needed)
PRETRAINED_PARAMS = "pretrained/single_object_angle_params.txt"

# ------------------------------------------------------------
# Load pretrained parameters
# ------------------------------------------------------------

param_dict = load_param_dict(PRETRAINED_PARAMS)

# Only freeze shape-related parameters
SHAPE_TOKENS = {"cone", "cube", "cylinder", "sphere"}

shape_param_dict = {
    k: v for k, v in param_dict.items()
    if any(tok in k for tok in SHAPE_TOKENS)
}

print(f"Loaded {len(shape_param_dict)} frozen shape parameters")

# ------------------------------------------------------------
# Apply freezing to sentence circuits
# ------------------------------------------------------------

frozen_train_sen = [
    freeze_circuit_from_dict(circ, shape_param_dict)
    for circ in train_sen
]
...
frozen_test_sen = [
    freeze_circuit_from_dict(circ, shape_param_dict)
    for circ in test_sen
]

# Replace original circuits
train_sen = frozen_train_sen
val_sen   = frozen_val_sen
test_sen  = frozen_test_sen
